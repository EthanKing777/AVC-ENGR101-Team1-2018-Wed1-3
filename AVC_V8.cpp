...
void direction(int error, int prevError) {
	int vGo = 47; /* speed when bot is moving forward */
	float Kp = -0.25; /* amount bot reacts to a change in the error, is proportional to amount is turns by */
	float Kd = -0.01;
	float diff = 0;
	float dv = 0;
