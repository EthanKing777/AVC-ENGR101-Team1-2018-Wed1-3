...
int main(){
	...
	for(i=0;i<1000;i=i+1) {
			if (findRed() == 1) {
				...
				if ( (np<20 || np>310) && count == 0) { //90 degree left turn
					turnLeft();
					count = 1;
					goStriaght(error, prevError, elasped, prevtime);
					/*gettimeofday(&t2,0);
					elasped = (t2.tv_sec - t1.tv_sec)*1000000 +  (t2.tv_usec - t1.tv_usec);
					gettimeofday(&t1,0);
					direction(error, prevError, elasped, prevtime);
					prevError = error;*/
				} else if ((np<20 || np>310) && count == 1) { //180 degree turn if no white found after left turn
					turnAround();
					count = 0;
					goStriaght(error, prevError, 
elasped, prevtime);
					/*gettimeofday(&t2,0);
					elasped = (t2.tv_sec - t1.tv_sec)*1000000 +  (t2.tv_usec - t1.tv_usec);
					gettimeofday(&t1,0);
