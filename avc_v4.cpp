...
Boolean findRed() {
	takepicture();
	int pix = get_pixel(120,160,0);//Red pixel.
	if (pix>=200) {
		return true;
	} else {
		return false;
...
int main(){
	...
	while (true) {
		...
		if (quad == 2) {
		if (findRed == true) {//See if it has reached the red tape
				quad = 3;
			}
			error = calcError();//Calculate error 
			int count = 0;
			if (np ==0 && count==0) {//If black turn left. 
				set_motor(2,50);
				sleep1(2,0);
				count = 1;
			} else if (np ==0 && count==1) {//If still black and turned left before turn right twice. 
				set_motor(1,50);
				sleep1(4,0);
				count = 0;
